...
import service from '../src/services/nunjucks'
...
describe('Nunjucks', function () {
  ...
  it('test plugin', async () => {
    ...
    expect(plugin).to.be.an.instanceof(NunjucksPlugin, 'Plugin is not an instance of NunjucksPlugin')

    const service = mid.getService('mid:nunjucks')
    expect(service).to.be.an.instanceof(NunjucksService, 'Service is not an instance of NunjucksService')
  })
